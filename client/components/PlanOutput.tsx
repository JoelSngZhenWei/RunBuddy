...
export default function PlanOutput({
    ...
    const showAllContent = focus === "output"
    const handleCopy = async () => {
        toast("Content copied", {
            description: "Placeholder copy function, not actually copied.",
            duration: 1500,
            icon: <Copy className="h-5 w-5" />
        })
    }
    const handleRegenerateResponse = async () => {
        toast("Regenerate response triggered", {
            description: "Placeholder regenerate response description.",
            duration: 1500,
